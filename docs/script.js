document.addEventListener('DOMContentLoaded', function() {
    ...
    // interview/index.htmlからの戻りかどうかをチェック
    const fromInterview = sessionStorage.getItem('fromInterview');
    
    if (!fromInterview) {
        // interviewからの戻りでない場合は、クレジットをリセット
        localStorage.removeItem('selectedCredits');
        resetAllCreditsToBlueSquare();
        console.log('クレジットをリセットしました');
    } else {
        // interviewからの戻りの場合は、保存されたクレジット状態を復元
        loadCreditsFromStorage();
        console.log('インタビューからの戻り: クレジット状態を復元しました');
    }
    ...
    // ページ読み込み時にdirectionを自動表示（interviewからの戻りではない場合のみ）
    if (!fromInterview) {
        setTimeout(() => {
    ...
    function stopSlots() {
        lists.forEach((list, index) => {
            setTimeout(() => {
                ...
                const selectedImg = newOrder[0].querySelector('img');
                if (selectedImg) {
                    const altText = selectedImg.alt;
    ...
    // 閉じるボタンのクリックイベント
    const directionCloseBtn = document.querySelector('.direction-close-btn');
    if (directionCloseBtn) {
        directionCloseBtn.addEventListener('click', function(e) {
            e.stopPropagation();
